  ...
  <div id="app" :class="{ 'no-sidebar': !userStore.isLoggedIn }">
    <header class="header">
      ...
      <div class="header-right">
        <el-tooltip effect="dark" :content="isDark ? '切换浅色模式' : '切换黑暗模式'" placement="bottom">
          <el-button class="theme-toggle" size="small" circle @click="toggleTheme">
...
.header {
  ...
  padding: 0 24px; height: 56px; background: var(--bg-card);
  box-shadow: 0 1px 4px rgba(0,0,0,0.06); z-index: 100; flex-shrink: 0;
  transition: background var(--transition);
}
.header-right { display: flex; align-items: center; gap: 12px; }
.theme-toggle { border: none; background: var(--bg-page); color: #606266; }
.theme-toggle:hover { color: var(--primary); background: var(--bg-page); }
html.dark .theme-toggle { color: #a0a3b1; }
...
.sidebar :deep(.el-menu) {
  border-right: none; height: 100%; padding-top: 8px;
...
.content { flex: 1; overflow-y: auto; background: var(--bg-page); transition: background var(--transition); }
